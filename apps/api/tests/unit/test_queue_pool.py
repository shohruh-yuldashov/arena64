...
def _pool(
    *,
    ...
    return QueuePool(variant=variant, queue_type=queue_type, region=region)
...
class TestConstruction:
    ...
    def test_the_region_defaults_to_global(self) -> None:
        ...
        assert QueuePool(variant=RUSSIAN, queue_type=QueueType.CASUAL).region is Region.GLOBAL
    ...
    def test_the_primitive_form_is_normalised_to_the_enum(self) -> None:
        ...
        pool = QueuePool(variant="russian_8x8", queue_type=QueueType.RANKED)  # type: ignore[arg-type]
    ...
    def test_the_refusal_happens_at_construction_and_not_at_use(self) -> None:
        ...
        with pytest.raises(VariantNotOffered):
            QueuePool(variant="draughts_64", queue_type=QueueType.CASUAL)  # type: ignore[arg-type]
...
class TestTheIdentifier:
    def test_it_reads_widest_to_narrowest(self) -> None:
        ...
        assert pool.identifier() == "russian_8x8:casual:europe"
    ...
    def test_it_carries_no_player_information(self) -> None:
        ...
        assert _pool().identifier().count(":") == 2
