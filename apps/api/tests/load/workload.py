...
class LegalGame:
    """The next legal move, from the engine that will judge it — §17.

    ## Why this replaces a hardcoded opening

    A64-028.5 drove games from a fixed six-ply opening and wrapped it. From
    ply three the moves were illegal, the server answered
    `game.move.rejected`, no broadcast was ever published — and the harness
    counted the absent broadcast as a **missing cross-instance frame**. That
    is how a benchmark reports a transport defect the platform does not
    have: 20 of 30 "lost" frames were moves the engine had correctly
    refused.

    So the sequence is generated by `MoveGenerator` against a position this
    class advances with `MoveApplier` — the same two classes the server
    uses. Deterministic (always the first legal move), arbitrarily long, and
    legal by construction rather than by hope.
    """

    def __init__(self, variant: BoardVariant = BoardVariant.RUSSIAN_8X8) -> None:
        from app.modules.engine import MoveGenerator, MoveValidator, PlayerSide, Position
        ...
        self._applier = MoveApplier(MoveValidator(self._generator))
        self._position = Position(board=initial_board(variant), side_to_move=PlayerSide.LIGHT)

    @property
    def side_to_move(self) -> str:
        side: str = self._position.side_to_move.value
        return side

    def next_path(self) -> list[str] | None:
        """The squares of the next legal move, or `None` if the game is over."""
        moves = self._generator.legal_moves(self._position)
        if not moves:
            return None
        return [str(square) for square in moves[0].path]

    def advance(self) -> None:
        """Applies the move `next_path` returned, keeping this in step with
        the server's own state."""
        moves = self._generator.legal_moves(self._position)
        if moves:
            self._position = self._applier.apply(self._position, moves[0])


async def seeded_cohort(size: int, *, prefix: str = "") -> list[Player]:
    """`size` accounts, created in PostgreSQL and given real access tokens.

    ## Why not the register and login endpoints — §28, §51
    ...
    **Nothing under test is weakened by this.** These are real tokens: the
    ...
    among the things being measured.
    """
